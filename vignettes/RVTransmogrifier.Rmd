...
```{r}
SUMMER2016 <- getSurvey("SUMMER", years = 2016, code=10)
nrow(SUMMER2016$GSINF)
```
...
paste("SUMMER2016$GSMISSIONS:",nrow(SUMMER2016$GSMISSIONS))
paste("SUMMER2016$GSINF:",nrow(SUMMER2016$GSINF))
paste("SUMMER2016$GSCAT:",nrow(SUMMER2016$GSCAT))
paste("SUMMER2016$dataDETS:",nrow(SUMMER2016$dataDETS))
```


3. Merge together appropriate objects...
```{r}
cod_2016 <- merge(catch_cod_2016, sets_2016, all.y=T, by=c("MISSION", "SETNO"))
```
...
# add cod data
codPlot <- codPlot + ggplot2::geom_point(data=cod_2016[!is.na(cod_2016$TOTWGT),], aes(x = SLONG_DD, y=SLAT_DD, size=TOTWGT), colour="blue")
#add nullsets
codPlot <- codPlot + ggplot2::geom_point(data=cod_2016[is.na(cod_2016$TOTWGT),], aes(x = SLONG_DD, y=SLAT_DD), shape = 3, colour="black", size = 2)
# add some land
...
limits<- c(range(cod_2016[!is.na(cod_2016$TOTWGT),"SLONG_DD"]),range(cod_2016[!is.na(cod_2016$TOTWGT),"SLAT_DD"]))
...
```{r, fig.width = 10}
codPlot <- codPlot + ggplot2::geom_point(data=cod_2016[cod_2016$TYPE!=1,], aes(x = SLONG_DD, y=SLAT_DD), shape = 4, colour="red", size = 4)
codPlot
```
...
head(SUMMER2016$GSCAT)
head(SUMMER2016$GSINF)
head(SUMMER2016$GSSPECIES)
```
...
plotRV(SUMMER2016, code=10, plotSets = "TOTWGT", plotBathy = "FILL")  
```
...
for (s in species){
  spPlot <- plotRV(SUMMER2016, code=s, plotSets = "TOTWGT", plotBathy = "FILL")
  print(spPlot)
}
```
